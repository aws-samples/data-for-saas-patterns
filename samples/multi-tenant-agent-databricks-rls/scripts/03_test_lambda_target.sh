...
# 03_test_lambda_target.sh — Test the Lambda target path end-to-end.
#
# Verifies: Gateway → Interceptor (x-tenant-id) → Lambda (WIF) → Databricks (Row Filter)
...
echo "  Target:  query-databricks___query_databricks"
echo ""
...
echo "  SQL: SELECT * FROM saas_workshop.saas_pilot.orders"
RESULT_1=$(curl -s -X POST "$GATEWAY_URL" \
  -H "Authorization: Bearer $TOKEN_1" \
  -H "Content-Type: application/json" \
  -H "Mcp-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"query-databricks___query_databricks","arguments":{"sql":"SELECT * FROM saas_workshop.saas_pilot.orders"}},"id":1}')
...
RESULT_2=$(curl -s -X POST "$GATEWAY_URL" \
  ...
  -H "Content-Type: application/json" \
  -H "Mcp-Protocol-Version: 2025-11-25" \
  -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"query-databricks___query_databricks","arguments":{"sql":"SELECT * FROM saas_workshop.saas_pilot.orders"}},"id":2}')
...
if [[ "$COUNT_1" == "3" && "$COUNT_2" == "2" ]]; then
  echo "  ✅ PASS — Same SQL, different data. Lambda target row filter isolation confirmed."
else
