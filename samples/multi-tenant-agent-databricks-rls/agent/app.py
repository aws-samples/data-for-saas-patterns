...
"""
app.py — Multi-Tenant Databricks Agent (Strands on AgentCore Runtime)
...
Tenant isolation is enforced by the Gateway + Interceptor + Databricks Unity Catalog row filters.
...
SYSTEM_PROMPT = """You are a helpful data analyst assistant for a multi-tenant SaaS application.
You have access to Databricks tools. The workspace uses Unity Catalog.
Available tables: saas_workshop.saas_pilot.orders, saas_workshop.saas_pilot.customer_data

Important:
- You do NOT need to filter by tenant_id in your SQL queries.
- Unity Catalog row filters automatically ensure you only see data belonging to the current tenant.
- Write simple, clean SQL without WHERE tenant_id = ... clauses.
- Always use the query_databricks tool to answer data questions.

When users ask about their data, use the query tools to fetch results and present them clearly.
"""

app = FastAPI(title="Multi-Tenant Databricks Agent", version="1.0.0")
